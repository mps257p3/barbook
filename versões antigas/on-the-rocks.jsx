...
// ─── MIGRAÇÃO barbook → on-the-rocks ─────────────────────────────────────────
const MIGRATION_KEY = "on-the-rocks_migrated_v1";
if (!localStorage.getItem(MIGRATION_KEY)) {
  const keys = ["custom","favs","owned","tried","spirits","overrides"];
  keys.forEach(k => {
    const old = localStorage.getItem(`barbook_${k}`);
    if (old !== null) {
      localStorage.setItem(`on-the-rocks_${k}`, old);
      localStorage.removeItem(`barbook_${k}`);
    }
  });
...
export default function OnTheRocks(){
  const [customRecipes, setCustomRecipes] = useState(() => {
    try { return JSON.parse(localStorage.getItem("on-the-rocks_custom")||"[]"); } catch { return []; }
  });
  const [favs, setFavs] = useState(() => {
    try { return JSON.parse(localStorage.getItem("on-the-rocks_favs")||"[]"); } catch { return []; }
  });
  const [owned, setOwned] = useState(() => {
    try { return JSON.parse(localStorage.getItem("on-the-rocks_owned")||"[]"); } catch { return []; }
  });
  const [tried, setTried] = useState(() => {
    try { return JSON.parse(localStorage.getItem("on-the-rocks_tried")||"[]"); } catch { return []; }
  });
  const [customSpirits, setCustomSpirits] = useState(() => {
    try { return JSON.parse(localStorage.getItem("on-the-rocks_spirits")||"[]"); } catch { return []; }
  });
  const [overrides, setOverrides] = useState(() => {
    try { return JSON.parse(localStorage.getItem("on-the-rocks_overrides")||"{}"); } catch { return {}; }
  });
  useEffect(()=>{ try{localStorage.setItem("on-the-rocks_custom",JSON.stringify(customRecipes));}catch{} },[customRecipes]);
  useEffect(()=>{ try{localStorage.setItem("on-the-rocks_favs",JSON.stringify(favs));}catch{} },[favs]);
  useEffect(()=>{ try{localStorage.setItem("on-the-rocks_owned",JSON.stringify(owned));}catch{} },[owned]);
  useEffect(()=>{ try{localStorage.setItem("on-the-rocks_tried",JSON.stringify(tried));}catch{} },[tried]);
  useEffect(()=>{ try{localStorage.setItem("on-the-rocks_spirits",JSON.stringify(customSpirits));}catch{} },[customSpirits]);
  useEffect(()=>{ try{localStorage.setItem("on-the-rocks_overrides",JSON.stringify(overrides));}catch{} },[overrides]);
  ...
  const exportJSON = () => {
    ...
    a.download = `on-the-rocks_backup_${new Date().toISOString().slice(0,10)}.json`;
    a.click();
  };
